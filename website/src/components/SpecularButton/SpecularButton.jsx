...
const SpecularButton = ({
  ...
  const rootClassName = `specular-button specular-button--${size}${className ? ` ${className}` : ''}`;
  const rootStyle = {
    '--sb-radius': `${radius}px`,
    '--sb-tint': tint,
  ...
  // Link mode: plain <a> for file downloads (Link prefetch is pointless for
  // static assets), Next.js <Link> for in-app navigation.
  if (href) {
    const Tag = download ? 'a' : Link;
    return (
      <Tag
        ref={btnRef}
        href={href}
